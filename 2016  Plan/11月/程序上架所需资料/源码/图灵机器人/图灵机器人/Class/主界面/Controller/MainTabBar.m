...
-(void)setupTabBar
{
    ...
    [self setupVCTabbar:[MainViewController new] andImage:@"turing" andSelectImage:@"turing_selec" andTitle:@"图灵"];
...
-(void)setupVCTabbar:(UIViewController*)view andImage:(NSString*)image andSelectImage:(NSString *)selectImage andTitle:(NSString*)title
{
    if ([title isEqualToString:@"图灵"]) {
        UIImage * normalImage = [[UIImage imageNamed:@"tabBar_essence_icon"]  imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
        UIImage * selectImage = [[UIImage imageNamed:@"tabBar_essence_click_icon"]  imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
