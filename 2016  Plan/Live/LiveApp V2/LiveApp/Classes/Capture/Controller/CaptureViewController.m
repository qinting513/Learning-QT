...
//    10.添加视频预览图层
    AVCaptureVideoPreviewLayer *preViewLayer = [AVCaptureVideoPreviewLayer
                                                layerWithSession:_captureSession];
...
- (IBAction)toggleCapture:(id)sender {
   ...
   // 获取需要改变的方向
    ...
    // 获取改变的摄像头输入设备
    AVCaptureDeviceInput *toggleDeviceInput = [AVCaptureDeviceInput
...
//    NSLog(@"聚焦====================");
    ...
    // 把当前位置转换为摄像头点上的位置
    CGPoint cameraPotin = [_preViewLayer captureDevicePointOfInterestForPoint:point];
    
    /** 设置聚焦光标位置 */
    [self setFocusCursorWithPoint:point];
    // 设置自动聚焦 自动曝光
    [self focusWithMode:AVCaptureFocusModeAutoFocus
           exposureMode:AVCaptureExposureModeAutoExpose atPoint:cameraPotin];
}
 ...
 *  设置聚焦
 */
-(void)focusWithMode:(AVCaptureFocusMode)focusMode exposureMode:(AVCaptureExposureMode)
            exposureMOde atPoint:(CGPoint)point{
